      ...
      {EXPERIENCES.map((exp, idx) => {
        ...
        return (
          <div key={exp.id} className="relative grid grid-cols-[1fr_auto_1fr] gap-4 md:gap-8 items-center mb-12 last:mb-0">
            {/* Left slot */}
            <div className={`text-right ${isLeft ? '' : 'hidden md:block md:invisible pointer-events-none'}`}>
              {isLeft && Card}
            </div>
            ...
            {/* Right slot */}
            <div className={`text-left ${!isLeft ? '' : 'hidden md:block md:invisible pointer-events-none'}`}>
              {!isLeft && Card}
            </div>
          </div>
        );
